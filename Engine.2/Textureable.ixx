...
public:
		...
		: folder_path(_path), current_tex_file(_file), current_face(XA::RIGHT, YA::UP)
	{
		...
		_object.get<sf::Sprite>()->scale(abs(_scale.x), abs(_scale.y));
	...
	{
		setObject(object_type::SPRITE);
		_object.get<sf::Sprite>()->scale(abs(_scaleY), abs(_scaleY));
	...
	void change_file(std::filesystem::path _file) {
		...
		_object.get<sf::Sprite>()->setOrigin(tex.getSize().x / 2.f, tex.getSize().y / 2.f);
	...
	sf::Vector2f getTextureSize() {
		return _object.get<sf::Sprite>()->getGlobalBounds().getSize();
	}

	void turn_sprite_x(XA x) {
		if (current_face.first != x)
			_object.get<sf::Sprite>()->scale(-1.f, 1.f);
